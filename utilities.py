...
def pretraining(policy_PT, params, dtime, tf, states_n, control_inputs, runs_PT,
    ...
    for i_episode in range(runs_PT):
        ...
        for step_j in range(t_steps):
            ...
            for state_cntrl in range(len(controls)):
                rand = np.random.uniform(-pert_size, pert_size)
                controls[state_cntrl][step_j] = controls[state_cntrl][step_j] * (1. + rand)
    ...
    for x_i in range(len(x_data)):  # episode
        for x_ii in range(len(x_data[x_i])):  # step
            ...
            # x_data[x_i][x_ii]=tuple(x_data[x_i][x_ii])  # excludes time as a state
...
def compute_run(policy_CR, params_CR, dtime_CR, tf_CR, states_n_CR,
                control_n_CR, t_steps_CR, runs_CR, std_sqr,
                initial_state_CR=np.array([1, 0]), plot_CR=False):
    if t_steps_CR != tf_CR / dtime_CR:
        print('t_steps and tf/dtime do not match t_step = ',
              t_steps_CR, ' tf/dtime = ', tf_CR / dtime_CR)
    if plot_CR:
    ...
    for step_j in range(t_steps_CR):
        ...
        final_state = model_integrator.model_integration(
            params_CR, initial_state_I, contrl, dtime_CR)
